...
def missing_user_content(content):
  if content == "":
    return "So what are you trying to say? You forgot to enter your comment!"

def missing_user_name(name):
  if name == "":
    return "Have we met? You forgot to enter your name!"

def missing_user_email(email):
  if email == "":
    return "Look, we promise no spam. You forgot to enter your email!"
...
class MainHandler(Handler):
  def write_form(self, errors="", content="", name="", email=""):
  ...
  def post(self):
    ...
    #validations
    user_content = missing_user_content(content)
    user_name = missing_user_name(name)
    user_email = missing_user_email(email)

    print user_content, user_name, user_email

    if not (user_content and user_name and user_email):
      self.render("coursenotes.html",
      cards=cards,
      comments=comments,
      errors=errors)
    ...
    if content and author:
      ...
      self.redirect('/?errors=Please fill out name and comment sections!')
    else:
      self.redirect('/')


class AddNotesHandler(Handler):
  def get(self):
